 ...
 * @author Kevin Wayne
 */
public class BST<Key> implements OrderStatisticTree<Key> {
	 ...
	 * @param cmpt
	 */
	public BST(Comparator<Key> cmpt) {
		this.cmpt = cmpt;
	}
...
//		else              x.val   = val;
	...
	private boolean isBST(Node x, Key min, Key max) {
		...
		if (min != null && cmpt.compare(x.key, min) <= 0) return false;
		if (max != null && cmpt.compare(x.key, max) >= 0) return false;
	...
	private boolean isRankConsistent() {
		...
		for (Key key : keys())
			if (cmpt.compare(key, select(rank(key))) != 0) return false;
		return true;
	}
...
//			if (!(o instanceof Node)) return false;
		...
		@Override
		public int hashCode() {
			return Objects.hash(key, left, right, size);
		}
	}
}