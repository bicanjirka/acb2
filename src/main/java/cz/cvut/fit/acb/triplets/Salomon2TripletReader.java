...
public class Salomon2TripletReader extends BaseTripletReader {

	public Salomon2TripletReader(int distBits, int lengBits, InputStream in) {
	...
	public Triplet read() throws IOException {
		...
		if (flag == 0) {
			...
			int ch = in.read(); // TODO different encoding
			t = eof(flag, read, length, ch) ? null : new Triplet(distance, length, (char) ch);
		}
